...
RSpec.describe 'EventEntries', type: :request do
  ...
  let!(:team_member3) { FactoryBot.create(:team_member3, team: other_team, user: user) }
  let!(:event) { FactoryBot.create(:event) }
  let!(:other_team_event) { FactoryBot.create(:other_team_event) }
  let!(:event_entry) { EventEntry.find_by(user_id: user, event_id: event) }
  let!(:other_team_event_entry) { EventEntry.find_by(user_id: user, event_id: other_team_event) }
  ...
  describe '他のユーザーのスケジュール出欠の編集、更新が失敗する' do
    it 'スケジュール出欠編集からリダイレクトされる' do
      ...
      get edit_event_entry_path(event_entry)
      expect(response).to redirect_to event_path(other_team)
    end
